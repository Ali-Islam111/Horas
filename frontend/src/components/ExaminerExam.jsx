        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">{t('examiner.exams.welcomeDesc')}</p>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto flex items-center gap-8 overflow-x-auto no-scrollbar">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 py-4 text-sm font-semibold transition-colors relative ${tab.id === 'examinerExam' ? 'text-purple-400' : 'text-slate-400 hover:text-slate-200'}`}
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                {tab.icon}
              </svg>
        ...
        <div className="rounded-2xl border border-white/5 bg-white/5 p-4 backdrop-blur-md mb-6 inline-flex overflow-x-auto no-scrollbar max-w-full">
          <div className="flex gap-2 bg-slate-950/50 border border-white/5 rounded-xl p-1">
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${filterStatus === 'all' ? 'bg-white/10 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
                }`}
            >
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${filterStatus === 'scheduled' ? 'bg-emerald-500/20 text-emerald-300 shadow-sm' : 'text-slate-400 hover:text-slate-200'
                }`}
            >
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${filterStatus === 'ongoing' ? 'bg-cyan-500/20 text-cyan-300 shadow-sm' : 'text-slate-400 hover:text-slate-200'
                }`}
            >
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${filterStatus === 'completed' ? 'bg-orange-500/20 text-orange-300 shadow-sm' : 'text-slate-400 hover:text-slate-200'
                }`}
            >
            ...
            <div key={exam.id} className="group flex flex-col justify-between rounded-2xl border border-white/5 bg-white/5 p-6 backdrop-blur-md hover:border-purple-500/30 hover:bg-white/[0.07] transition-all duration-500 relative overflow-hidden">
              ...
              <div className="relative z-10 flex items-start justify-between mb-6">
                <div className="flex-1 pr-4">
                  ...
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${exam.status === 'scheduled' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                    exam.status === 'ongoing' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' :
                      'bg-orange-500/10 text-orange-400 border-orange-500/20'
                    }`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${language === 'ar' ? 'ml-1.5' : 'mr-1.5'} ${exam.status === 'scheduled' ? 'bg-emerald-400' :
                      exam.status === 'ongoing' ? 'bg-cyan-400' : 'bg-orange-400'
                      }`}></span>
